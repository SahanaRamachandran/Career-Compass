    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {results.match_explanation && (
        <div className="card bg-blue-500/10 border-blue-500/30">
          <h3 className="text-xl font-bold text-slate-200 mb-3 flex items-center">
            <svg className="w-6 h-6 mr-2 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Why This Score?
          </h3>
          <p className="text-slate-300">{results.match_explanation}</p>
        </div>
      )}
        ...
        <div className="card">
          ...
          <div className="bg-purple-500/20 p-4 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <span className="text-lg font-semibold text-slate-200">Level: {results.role_suitability.level}</span>
            ...
            <p className="text-slate-300 text-sm">{results.role_suitability.reasoning}</p>
          </div>
        </div>
      )}
      ...
      {(results.keywords_found?.length > 0 || results.keywords_missing?.length > 0) && (
        <div className="card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <p className="text-sm font-medium text-slate-300 mb-2">✓ Keywords Found:</p>
            ...
            <div>
              <p className="text-sm font-medium text-slate-300 mb-2">✗ Missing Keywords:</p>
              <div className="flex flex-wrap gap-2">
          ...
          {results.keyword_density_score !== undefined && (
            <div className="mt-4">
              <p className="text-sm font-medium text-slate-300 mb-2">Keyword Density Score:</p>
              ...
              <p className="text-xs text-slate-400 mt-1">{results.keyword_density_score}% of important keywords present</p>
            </div>
          )}
        </div>
      )}
            ...
            {Object.entries(results.resume_sections_analysis).map(([section, data]) => (
              <div key={section} className="border-l-4 border-blue-500 pl-4 py-2">
                <div className="flex items-center justify-between mb-1">
                  <span className="font-semibold text-slate-200 capitalize">{section}</span>
